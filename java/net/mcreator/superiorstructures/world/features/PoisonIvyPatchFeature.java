...
import net.mcreator.superiorstructures.procedures.IvySpreadProcedure;

import java.util.Set;
import java.util.List;

public class PoisonIvyPatchFeature extends Feature<NoneFeatureConfiguration> {
	private final Set<ResourceKey<Level>> generate_dimensions = Set.of(Level.OVERWORLD);
	private final List<Block> base_blocks;
	private StructureTemplate template = null;

	public PoisonIvyPatchFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(ResourceLocation.fromNamespaceAndPath("superiorstructures", "poison_ivy"));
		if (template == null)
			return false;
		...
		if ((context.random().nextInt(1000000) + 1) <= 60000) {
			...
			for (int a = 0; a < count; a++) {
				...
				if (template.placeInWorld(context.level(), spawnTo, spawnTo, new StructurePlaceSettings().setMirror(Mirror.values()[context.random().nextInt(2)]).setRotation(Rotation.values()[context.random().nextInt(3)]).setRandom(context.random())
					...
					IvySpreadProcedure.execute(world, x, y, z);
					anyPlaced = true;
				}
			}
		}
