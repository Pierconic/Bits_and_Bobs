...
@EventBusSubscriber
public class PotionOfShockingRecipeBrewingRecipe implements IBrewingRecipe {
	@SubscribeEvent
	public static void init(RegisterBrewingRecipesEvent event) {
		event.getBuilder().addRecipe(new PotionOfShockingRecipeBrewingRecipe());
	}
	...
	public boolean isIngredient(ItemStack ingredient) {
		return Ingredient.of(new ItemStack(SuperiorstructuresModItems.LIGHTNING_SHARD.get())).test(ingredient);
	}

	@Override
	public ItemStack getOutput(ItemStack input, ItemStack ingredient) {
		if (isInput(input) && isIngredient(ingredient)) {
			return PotionContents.createItemStack(input.getItem(), SuperiorstructuresModPotions.SHOCKING_POTION);
		}
