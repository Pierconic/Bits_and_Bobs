...
public class FrostBlasterFiredProcedure {
		...
		if (itemstack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getDouble("charge") > 0) {
			if (time < 80) {
					...
					if (!projectileLevel.isClientSide()) {
						Projectile _entityToSpawn = new Object() {
							public Projectile getArrow(Level level, Entity shooter, float damage, int knockback, byte piercing) {
								AbstractArrow entityToSpawn = new FrostBlasterProjectileEntity(SuperiorstructuresModEntities.FROST_BLASTER_PROJECTILE.get(), level) {
									@Override
				...
				if (entity instanceof Player _player)
					_player.getCooldowns().addCooldown(itemstack.getItem(), 36);
