...
public class EnergyBlasterFiredProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, ItemStack itemstack, double time) {
		...
		if (itemstack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getDouble("charge") > 0) {
			if (time < 80) {
				ArbitraryKickbackProcedure.execute(entity, 0.15);
					...
					if (!projectileLevel.isClientSide()) {
						Projectile _entityToSpawn = new Object() {
							public Projectile getArrow(Level level, Entity shooter, float damage, int knockback, byte piercing) {
								AbstractArrow entityToSpawn = new EnergyBlasterProjectileEntity(SuperiorstructuresModEntities.ENERGY_BLASTER_PROJECTILE.get(), level) {
									@Override
				...
				ArbitraryKickbackProcedure.execute(entity, 0.05);
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, BlockPos.containing(entity.getX(), entity.getY(), entity.getZ()), BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("superiorstructures:blasterfire")), SoundSource.NEUTRAL, 1, 1);
					} else {
				...
				}
				{
					...
					if (!projectileLevel.isClientSide()) {
						...
						}.getArrow(projectileLevel, entity, 2, 0, (byte) 0);
						_entityToSpawn.setPos(_shootFrom.getX(), _shootFrom.getEyeY() - 0.1, _shootFrom.getZ());
						_entityToSpawn.shoot(_shootFrom.getLookAngle().x, _shootFrom.getLookAngle().y, _shootFrom.getLookAngle().z, (float) 1.6, 0);
						projectileLevel.addFreshEntity(_entityToSpawn);
					}
