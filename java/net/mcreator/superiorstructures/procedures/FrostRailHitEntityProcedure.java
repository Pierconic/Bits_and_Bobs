...
public class FrostRailHitEntityProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, Entity sourceentity) {
		...
		if (!(entity == sourceentity)) {
				...
				_level.sendParticles(ParticleTypes.SNOWFLAKE, x, y, z, 15, 0.2, 0.2, 0.2, 0.15);
			{
				...
				for (Entity entityiterator : _entfound) {
					if (entityiterator instanceof Blaze || entityiterator instanceof MagmaCube || entityiterator instanceof WitherSkeleton || entityiterator instanceof Strider || entityiterator instanceof Piglin
						...
						entityiterator.setTicksFrozen(720);
						if (entityiterator instanceof LivingEntity _entity && !_entity.level().isClientSide())
							_entity.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SLOWDOWN, Mth.nextInt(RandomSource.create(), 360, 480), 2, false, true));
					} else {
						entityiterator.setTicksFrozen(680);
						if (entityiterator instanceof LivingEntity _entity && !_entity.level().isClientSide())
			...
			SmallFrostBubbleProcedure.execute(world, x, y, z);
		}
	}
}
