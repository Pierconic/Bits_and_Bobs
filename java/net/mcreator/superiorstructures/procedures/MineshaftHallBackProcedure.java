...
public class MineshaftHallBackProcedure {
			...
			if (world.getBiome(BlockPos.containing(((Entity) world.getEntitiesOfClass(MineshaftCenterEntity.class, AABB.ofSize(new Vec3(x, y, z), 240, 240, 240), e -> true).stream().sorted(new Object() {
				...
				if (Math.random() < 0.75) {
					if (number == 0) {
						...
						if (world instanceof ServerLevel _serverworld) {
							StructureTemplate template = _serverworld.getStructureManager().getOrCreate(ResourceLocation.fromNamespaceAndPath("superiorstructures", "mineshaftiron_hall7"));
							if (template != null) {
								template.placeInWorld(_serverworld, BlockPos.containing(x, y, z), BlockPos.containing(x, y, z), new StructurePlaceSettings().setRotation(Rotation.CLOCKWISE_180).setMirror(Mirror.NONE).setIgnoreEntities(false), _serverworld.random, 3);
							}
						}
