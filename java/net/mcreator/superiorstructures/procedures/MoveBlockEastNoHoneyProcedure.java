...
public class MoveBlockEastNoHoneyProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		if (EastPushCheckProcedure.execute(world, x, y, z) && !((world.getBlockState(BlockPos.containing(x, y, z))).getBlock() == Blocks.HONEY_BLOCK)) {
			{
				BlockPos _pos = BlockPos.containing(x + 1, y, z);
				Block.dropResources(world.getBlockState(_pos), world, BlockPos.containing(x + 1.5, y + 0.5, z + 0.5), null);
			...
			{
				final Vec3 _center = new Vec3((x + 1.5), (y + 0.5), (z + 0.5));
				List<Entity> _entfound = world.getEntitiesOfClass(Entity.class, new AABB(_center, _center).inflate(1 / 2d), e -> true).stream().sorted(Comparator.comparingDouble(_entcnd -> _entcnd.distanceToSqr(_center))).toList();
				for (Entity entityiterator : _entfound) {
					{
						Entity _ent = entityiterator;
