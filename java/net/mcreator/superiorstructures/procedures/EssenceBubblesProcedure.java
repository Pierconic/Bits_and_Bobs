...
public class EssenceBubblesProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, BlockState blockstate) {
		if (world.isEmptyBlock(BlockPos.containing(x, y + 1, z)) && (blockstate.getFluidState().isSource() && Math.random() < 0.00225 || Math.random() < 0.0015)) {
			if (world instanceof Level _level) {
				if (!_level.isClientSide()) {
					_level.playSound(null, BlockPos.containing(x, y, z), BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("superiorstructures:essence.bubble")), SoundSource.NEUTRAL, 1,
							(float) Mth.nextDouble(RandomSource.create(), 0.95, 1.05));
				} else {
					_level.playLocalSound(x, y, z, BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("superiorstructures:essence.bubble")), SoundSource.NEUTRAL, 1, (float) Mth.nextDouble(RandomSource.create(), 0.95, 1.05), false);
				}
			}
		}
		if (Math.random() < 0.01 && world.isEmptyBlock(BlockPos.containing(x, y + 1, z)) && blockstate.getFluidState().isSource()) {
			if (Math.random() < 0.05) {
				...
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, BlockPos.containing(x, y, z), BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("superiorstructures:essence.plume")), SoundSource.NEUTRAL, 1, 1);
					} else {
						_level.playLocalSound(x, y, z, BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("superiorstructures:essence.plume")), SoundSource.NEUTRAL, 1, 1, false);
					}
				}
			} else {
