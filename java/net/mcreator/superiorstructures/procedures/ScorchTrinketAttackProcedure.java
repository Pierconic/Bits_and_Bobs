...
@EventBusSubscriber
public class ScorchTrinketAttackProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity, Entity sourceentity) {
		...
		double amps = 0;
		double trinkets = 0;
		if (sourceentity instanceof Player _playerHasItem ? _playerHasItem.getInventory().contains(new ItemStack(SuperiorstructuresModItems.SCORCH_TRINKET.get())) : false) {
			amps = 1;
			trinkets = 1;
			...
			if (sourceentity.getCapability(Capabilities.ItemHandler.ENTITY, null) instanceof IItemHandlerModifiable _modHandlerIter) {
				for (int _idx = 0; _idx < _modHandlerIter.getSlots(); _idx++) {
					ItemStack itemstackiterator = _modHandlerIter.getStackInSlot(_idx).copy();
					if (itemstackiterator.getItem() == SuperiorstructuresModItems.SCORCH_TRINKET.get() && trinkets < 3) {
						trinkets = trinkets + 1;
					}
				}
			}
			if (Math.random() < 0.1 * trinkets) {
				entity.igniteForSeconds((int) (3 * amps));
