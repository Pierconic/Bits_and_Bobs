...
public class DispenseFrostChargeProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Direction direction) {
		if (direction == null)
			return;
		if (direction == Direction.UP) {
			world.levelEvent(2001, BlockPos.containing(x, y + 1, z), Block.getId(Blocks.PACKED_ICE.defaultBlockState()));
		...
		if (direction == Direction.NORTH) {
			...
			world.levelEvent(2001, BlockPos.containing(x, y, z - 1), Block.getId(Blocks.PACKED_ICE.defaultBlockState()));
			if (world instanceof ServerLevel projectileLevel) {
				Projectile _entityToSpawn = new Object() {
					public Projectile getArrow(Level level, float damage, int knockback, byte piercing) {
						AbstractArrow entityToSpawn = new ThrownFrostChargeEntity(SuperiorstructuresModEntities.THROWN_FROST_CHARGE.get(), level) {
		...
		if (direction == Direction.EAST) {
			...
			world.levelEvent(2001, BlockPos.containing(x + 1, y, z), Block.getId(Blocks.PACKED_ICE.defaultBlockState()));
			if (world instanceof ServerLevel projectileLevel) {
				Projectile _entityToSpawn = new Object() {
					public Projectile getArrow(Level level, float damage, int knockback, byte piercing) {
						AbstractArrow entityToSpawn = new ThrownFrostChargeEntity(SuperiorstructuresModEntities.THROWN_FROST_CHARGE.get(), level) {
		...
		if (world instanceof Level _level) {
			if (!_level.isClientSide()) {
				_level.playSound(null, BlockPos.containing(x, y, z), BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("superiorstructures:frostblasterfire")), SoundSource.NEUTRAL, 4, 1);
			} else {
				_level.playLocalSound(x, y, z, BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.parse("superiorstructures:frostblasterfire")), SoundSource.NEUTRAL, 4, 1, false);
			}
		}
	}
}
