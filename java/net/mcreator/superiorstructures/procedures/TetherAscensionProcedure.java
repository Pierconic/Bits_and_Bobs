		...
		for (int index0 = 0; index0 < 7; index0++) {
			...
			for (int index1 = 0; index1 < 7; index1++) {
				...
				for (int index2 = 0; index2 < 7; index2++) {
					if ((world.getBlockState(BlockPos.containing(x + sx, y + sy, z + sz))).getBlock() == SuperiorstructuresModBlocks.ADAMANT.get()) {
			...
			for (Entity entityiterator : _entfound) {
				if (entityiterator instanceof Player) {
					if (entityiterator instanceof LivingEntity _entity && !_entity.level().isClientSide())
						_entity.addEffect(new MobEffectInstance(SuperiorstructuresModMobEffects.ASCENSION, 20, 1, false, false));
				}
			}
		}
	}
