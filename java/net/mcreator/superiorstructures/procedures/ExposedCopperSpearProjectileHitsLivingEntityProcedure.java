...
public class ExposedCopperSpearProjectileHitsLivingEntityProcedure {
		...
		if (!(sourceentity == entity)) {
			if (Math.random() < 0.05) {
				if (world instanceof ServerLevel _level) {
					ItemEntity entityToSpawn = new ItemEntity(_level, x, y, z, new ItemStack(SuperiorstructuresModItems.WEATHERED_COPPER_SPEAR.get()));
		...
		if (entity instanceof LivingEntity _livEnt4 && _livEnt4.isBlocking()) {
			if ((entity instanceof LivingEntity _livEnt ? _livEnt.getOffhandItem() : ItemStack.EMPTY).getItem() == Items.SHIELD) {
				if (entity instanceof Player _player)
					_player.getCooldowns().addCooldown((entity instanceof LivingEntity _livEnt ? _livEnt.getOffhandItem() : ItemStack.EMPTY).getItem(), 90);
			}
