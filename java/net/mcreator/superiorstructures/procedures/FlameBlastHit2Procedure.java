...
public class FlameBlastHit2Procedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		if (world instanceof ServerLevel _level)
			_level.sendParticles(ParticleTypes.FLAME, x, y, z, Mth.nextInt(RandomSource.create(), 15, 20), 0.1, 0.1, 0.1, 0.3);
		...
		if (Math.random() < 0.7 && world.isEmptyBlock(BlockPos.containing(x + 1, y + 1, z)) && world.getBlockState(BlockPos.containing(x + 1, y, z)).canOcclude()) {
			if (ModList.get().isLoaded("ashenremains")) {
		...
		if (Math.random() < 0.7 && world.isEmptyBlock(BlockPos.containing(x, y + 1, z - 1)) && world.getBlockState(BlockPos.containing(x, y, z - 1)).canOcclude()) {
			if (ModList.get().isLoaded("ashenremains")) {
