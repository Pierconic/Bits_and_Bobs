...
public class MansionRightWallProcedure {
		...
		if (world.getLevelData().getGameRules().getBoolean(SuperiorstructuresModGameRules.TICK_GENBLOCKS) == true) {
			...
			SuperiorstructuresMod.queueServerWork(20, () -> {
				if (world instanceof ServerLevel _serverworld) {
					...
					if (template != null) {
						template.placeInWorld(_serverworld, BlockPos.containing(x, y + 78, z), BlockPos.containing(x, y + 78, z), new StructurePlaceSettings().setRotation(Rotation.CLOCKWISE_90).setMirror(Mirror.NONE).setIgnoreEntities(false), _serverworld.random, 3);
					}
				}
			});
		}
