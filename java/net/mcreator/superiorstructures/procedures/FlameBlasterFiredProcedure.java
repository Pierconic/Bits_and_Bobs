...
public class FlameBlasterFiredProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, ItemStack itemstack, double time) {
		...
		if (itemstack.getOrDefault(DataComponents.CUSTOM_DATA, CustomData.EMPTY).copyTag().getDouble("charge") > 0) {
			if (time < 80) {
					...
					if (!projectileLevel.isClientSide()) {
						Projectile _entityToSpawn = new Object() {
							public Projectile getArrow(Level level, Entity shooter, float damage, int knockback, byte piercing) {
								AbstractArrow entityToSpawn = new FlameBlasterProjectileEntity(SuperiorstructuresModEntities.FLAME_BLASTER_PROJECTILE.get(), level) {
									@Override
				...
				ArbitraryKickbackProcedure.execute(entity, 0.1);
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
